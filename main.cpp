...
int main(){
    ...
    int begintime[ordenumber];
    int finishtime[ordenumber];
    int foodnumbers[ordenumber];
    int states[ordenumber];//0未开始，1成功，2失败
    struct foodkind *link[ordenumber][5];
    for(i=0;i<ordenumber;i++) {
        begintime[i]=timeread();//读取各订单开始时间并转化为时间戳
        finishtime[i]=0;
        states[i]=0;
        scanf("%s",ch);
        j=correctcomb(ch,comb,combnumber);//找出订单对应套餐编号
        foodnumbers[i]=comb[j].kindnumber;
        for(k=0;k<comb[j].kindnumber;k++) {
            link[i][k]=comb[j].link[k];
        }
    }
    //当日营业
    k=0;
    ...
    int pretime=25200,nowtime=begintime[0],minustime,/*kk*/state=1;//0：旧订单结束，1：新订单开始，2：both
    while(k<ordenumber) {
        //制作食物
        ...
        if (state == 1 || state == 2) {//有新增订单
            ...
            if (systemstate == 1) {
                states[k] = 1;
                finishtime[k] = begintime[k];
                for (j = 0; j < foodnumbers[k]; j++) {
                    link[k][j]->number--;
                    if (link[k][j]->number < 0)
                        temp = -link[k][j]->number * link[k][j]->needtime - link[k][j]->currenttime+begintime[k];
                    finishtime[k] = (finishtime[k] > temp ? finishtime[k] : temp);
                }//处理并计算结束时间
                if (finishtime[k] > begintime[k]) {
                    remainorder++;

                    for (temp = 0; temp <= allowmax+2; temp++) {
                        if (check[temp] == 90000) {
                            check[temp] = finishtime[k];
                            break;
                        }
                    }
                }
...
//            nowtime = orde[k].begintime;
    ...
    //print
    for(i=0;i<ordenumber;i++) {
        if(states[i]==3)//orde[i]失败
            printf("Fail\n");
        else {//orde[i]成功
            //将时间戳转化为hh:mm:ss时间
            second1=finishtime[i]%60;
            second2=second1%10;
            second1=second1/10;
            minute1=(finishtime[i]%3600)/60;
            minute2=minute1%10;
            minute1=minute1/10;
            hour1=finishtime[i]/3600;
            hour2=hour1%10;
            hour1=hour1/10;
